...
df_features.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(f"{schema_name}.{feature_table}")
...
model_details = mlflow.register_model(f"dbfs:/databricks/mlflow-tracking/{experiment_id}/{run_id}/artifacts/model", model_name)
...
df_preds = df_features.withColumn('predictions', champion_model(*champion_model.metadata.get_input_schema().input_names()))
